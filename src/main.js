...
// Store debris globally
let debrisField = null;
let earthRotationGroup = null; // Add this to store the earth rotation group
...
async function init() {
    ...
    let _initStage = fp.flow(setScene, setCamera, setRender, setLights);
    _initStage();

    earthRotationGroup = setEarthObject(); // Store reference
    // Add meshes to earthObject (the inner group), not rotationObject
    earthObject.add(innerEarth());
    earthObject.add(earthMap(imgs[3]));
    earthObject.add(earthBuffer(imgs[2]));
    earthObject.add(spike());

    // ADD DEBRIS HERE
    debrisField = createDebris();
    
    await scene.add(universe(imgs[5]))
    await scene.add(createRings());
    await scene.add(debrisField);  // Add debris to scene
    await scene.add(earthRotationGroup) // Changed from earthRotation
    await scene.add(outerEarth(imgs[1]))
    
    // Store references in consts for access in render loop
    consts.scene = scene;
    consts.renderer = renderer;
    consts.earthObject = earthObject;
    consts.rotationObject = rotationObject;
...
function render() {
    ...
    // FIXED: Rotate the earthObject continuously (only when mouse is not down)
    if (!isMouseDown && scene.getObjectByName("earthObject")) {
        earthObject.rotation.y += 0.002; // Adjust speed: higher = faster
    }
    
    // ADD DEBRIS ROTATION HERE
    if (debrisField) {
        debrisField.rotation.y += 0.0005;
        debrisField.rotation.x += 0.0002;
    }

    // Original code for manual rotation
